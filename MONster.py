...
#
#
import numpy as np # for average calculations
from input_file_parsing import parse_calib
import monster_transform as mt # transform widgets/layout/functions
import monster_stitch as ms # stitch widgets/layout/functions
import monster_integrate as mi # integrate widgets/layout/functions
import monster_queueloader as mq # queue widgets/layout/functions
import os, traceback, ast, time, sys, getpass, datetime # various functions
from ClickableLineEdit import * # custom (overridden) line edit - click once for adding cursor, double click to select everything
...
class DetectorEditor(QWidget):
    ...
    # None -> None
    # Remove the selected detector from the list of detectors and from the Properties.csv file
    def removeDetector(self):
        ...
        if curRow >= 0:
            ...
            def rem():
                ...
                with open(self.pPath, 'rb') as infile:
                    reader =csv.reader(infile)
                    prop = dict(reader)
                prop['detectors'] = str(self.string_detectorlist)
                with open(self.pPath, 'wb') as outfile:
            ...
            return
    # None -> None
    # Adds all the functions that should be connected to each button's signal
    ...
    # None -> None
    # Add the detector, after checking information, to the Properties.csv file and to the current running list of detectors.
...
# This class is the class that governs all things that occur in the GUI window. Its wrapper class is Menu, which is called in main
class MONster(QTabWidget):
    def __init__(self):
        ...
        self.imageWidth = screenShape.height()/3.5 # width of qchi/stitch/integrate images
        self.properties_OK = False
        # generate GUI widgets for all tabs
        ...
        # initial checking to see if properties and MONster_Bookkeeping folder exist in user's home directory
        if not os.path.exists(self.mPath):
            os.makedirs(self.mPath)
        if not os.path.exists(self.bkPath):
            self.win = QWidget()
        ...
        with open(self.pPath, 'rb') as p:
            ...
            for k,v in prop.items():
                count += 1      
        # check for properties file corruption
        if count < 16:
            resetProperties()
        ...
        # initialize empty process threads to connect the abort buttons
        self.transformThread = TransformThread(self, None, None, None, None, False, 0)  # initialize the transform thread
    ...
    def updateUi(self):
        ...
        #self.q_min.returnPressed.connect(lambda: mt.integrateThreadStart(self))
        
        self.q_min.clicked.connect(lambda: self.q_min.selectAll())        
        
        #self.q_max.returnPressed.connect(lambda: mi.integrateThreadStart(self))
        
        self.q_max.clicked.connect(lambda: self.q_max.selectAll())
        
        #self.chi_min.returnPressed.connect(lambda: mi.integrateThreadStart(self))
        
        self.chi_min.clicked.connect(lambda: self.chi_min.selectAll())
        
        #self.chi_max.returnPressed.connect(lambda: mi.integrateThreadStart(self))
        ...
        # Load previous run's information and images for easy access
        with open(self.pPath, 'rb') as prop:
    ...
    # int -> None
    # Copies all the information from the transform tab to either the macro editor or the integrate tab. Index is the number corresponding to each tab in MONster.
    ...
    # list -> None
    # What should be printed to the console after a stitch thread is finished
    ...
    # list list list -> None
    # What should be printed to the console after a thread is finished
    ...
    # string -> None
    # Appends a string to each console and makes sure the console updates to the latest message; scrolls down
    ...
    def getDataSourceDirectoryPath(self):
        try:
            folderpath = str(QFileDialog.getExistingDirectory(self, 'Select your data path', self.home))
            if folderpath != '':
    ...
    def saveCalibAction(self):
        name = (os.path.join(self.home, 'cal-%s.calib') %(datetime.datetime.now().strftime('%Y-%m-%d--%H-%M-%S')))
    ...
    def setProcessedLocation(self):
        ...
        if path.endswith('/untitled'): # pyqt will sometimes create an "untitled" folder before adding "Processed _ ..."
    ...
    # float -> None
    # Increments all progress bars by a certain value.
    ...
    # int float -> None
    # Sets the bar progress to whatever value is passed in; the bartype refers to which progress bar; the val refers to the value that the progress bar should be set to
...
class ConsoleWindow(QWidget):
    def __init__(self, win):
        ...
        self.tPath = os.path.join(self.bkpath, "thisRun.txt")
        self.mPath = os.path.join(self.home, "macros")        
        with open(self.pPath, 'rb') as fil:
        ...
        def save():
            ...
            with open(self.pPath, 'wb') as fil:
...
class Menu(QMainWindow):
    ...
    # None -> None
    # Adds file and menu actions to the top of the window
    def updateUi(self):
        self.help_dialog = None
        bar = self.menuBar()
        ...
        detectors.triggered.connect(havemercy)
    ...
    # HelpDialog -> None
    # Take a help dialog reference passed in and adds it as an attribute to MONster
    def setHelpDialog(self, hd):
        self.help_dialog = hd
        self.form_widget.help_dialog = hd
        if not os.path.exists(self.tPath) and self.form_widget.properties_OK:
            self.help_dialog.show()
            self.help_dialog.raise_()        

    # None -> None
    # opens and brings the help dialog to the front
    ...
    # None -> None
    # Clears all properties except the detectors, which remain unchanged
    def clearProperties(self):
        try:
            with open(self.pPath, 'rb') as p:
                reader = csv.reader(p)
                prop = dict(reader)
            with open(self.pPath, 'wb') as p:
                writer = csv.writer(p)
                prop["t_data_source"] = ""
                prop["s_data_source"] = ""
                prop["i_data_source"] = ""
...
# None -> None
# Resets all the properties, including detectors. Detectors are set to what they were during 1.1 release. To add more default detectors, add them in the exact same format as they are in this function. 
def resetProperties():
    try:
        home = os.path.expanduser('~')
        bkpath = os.path.join(home, 'MONster_Bookkeeping')
        pPath = os.path.join(bkpath, "Properties.csv")
        with open(pPath, 'wb') as p:
            writer = csv.writer(p)
            prop = {"t_data_source" :  ""}
            prop["s_data_source"] = ""
            prop["i_data_source"] = ""
...
# None -> boolean
# Returns True if the properties don't have anything in them; false if not.
