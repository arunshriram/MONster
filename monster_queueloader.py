...
import csv, ast, json
curIndex = 1 # index for "process" for each thing added to the queue

# Defines a macro data type
class Macro():
    def __init__(self, filename, dictionary):
        self.filename = filename
        self.mdict = dictionary
        # mdict is a python dictionary, you can see the keys down below
...
# This class defines the window that edits/loads/saves macros. It's pretty long and annoying...
class MacroEditor(QWidget):
    ...
    # int -> None
    # Sets the stack index to whatever is selected by the left list index
    def display(self, i):
        self.Stack.setCurrentIndex(i)        
    
    # None -> None
    # Sets specific widgets disabled.
    ...
    # None -> None
    # Defines widgets and sets the layout for the welcome stack.
    def stack0UI(self):
    ...
    # None -> None
    # Defines widgets and sets the layout for the transform stack.
    def stack1UI(self):
    ...
    # list string int int -> None
    # Takes the appropriate information and creates a detector data type with all the loaded information, and adds this detector to whatever list is passed in
    def addDetectorToList(self, lst, name, width, height):
        det = Detector(name, width, height)
        lst.append(det)        
        
    # None -> None
    # Defines widgets and sets the layout for the stitch stack.
    def stack2UI(self):
    ...
    # None -> None
    # Defines widgets and sets the layout for the integrate stack.
    def stack3UI(self):
    ...
    # None -> None
    # Adds connections to functions for each button/widget in MacroEditor
    ...
    # None -> None
    # Sets the processed file destination for stitch
    ...
    # None -> None
    # Lets the user select their data source for stitch.
    ...
    # boolean -> None
    # Alterantes the self.fieldsChanged value between false and true.
    def setFieldsChanged(self, boo):
        self.fieldsChanged = boo
    
    # None -> None    
    # Loads the appropriate folder based on the  data source the user selects
    ...
    # None -> None
    # Loads the appropriate file(s) based on the data source the user selects
    ...
    # None -> None
    # retreives the calibration source that the user selects. 
    ...
    # None -> None
    # Sets the trasnform processed location based on what the user selects
    ...
    # None -> None
    # Sets the integrate processed location based on what the user selects
    ...
    # None -> None
    # Saves the custom calibration that the user enters as a calibration file
    ...
    # string -> None
    # Refers to MONster's addtoconsole function
    def addToConsole(self, message):
        self.windowreference.addToConsole(message)
        
    # None -> None
    # Checks all transform values to see if they're suitable
    ...
    # None -> None
    # Checks all integrate values to see if they're suitable
    ...
    # None -> None
    # Checks all stitch values to see if they're suitable
    ...
    # None -> None
    # Checks values, then saves the macro
    def saveMacro(self):
    ...
    # None -> None
    # Tries to load all values in from another macro
    ...
    # None -> None
    # Loads a preset calibration file
...
# None -> None
# Generates all the widgets for the "Queue loader" tab
...
# None -> None
# Generates the layout for the "Queue loader" tab
...
# None -> None
# Begins the queue, iterating over the self.macroQueue list.
def beginQueue(self):
...
def startTransformThread(self, dataFiles, calib_source, detectorData, processed_filedir, increment):
    ...
    self.transformThread = TransformThread(self, processed_filedir, calib_source, detectorData, dataFiles, False, increment)
...
# None -> None
# Adds the value in self.curMacro to the queue, after checking to make sure values are suitable and that it's saved
...
# None -> None 
# Adds functionality to the "+" button on the queue loader page
def addMacro(self):
    self.editor.show()
    self.editor.raise_()

# None -> None
# removes the selected macro from the queue widget and from the queue saved in memory
...
# None -> None
# Saves the entire queue as a json file
def saveQueue(self):
...
# None -> None
# Calculates the increment for each file for each macro
