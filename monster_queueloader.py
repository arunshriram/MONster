...
class MacroEditor(QWidget):
    ...
    def checkIntegrateValues(self):
        ...
        calib_source = str(self.int_calib_source.text())
        dx = len(str(self.int_dcenterx.text()))
        dy = len(str(self.int_dcentery.text()))
        dd = len(str(self.int_detectordistance.text()))
        wav = len(str(self.int_wavelength.text()))
        da = len(str(self.int_detect_tilt_alpha.text()))
        dp = len(str(self.int_detect_tilt_delta.text()))
        if (not os.path.exists(calib_source)) or dp == 0 or dy == 0 or dx == 0 or dd == 0 or wav == 0 or da == 0:
            displayError(self, "Was not able to locate integrate calibration information.")
            return
        ...
        try:
            ...
            return
        return (i_filenames, calib_source, i_proc_dir)
    ...
    def saveMacro(self):
        ...
        if not workflow:
            ...
            if integrate:
                try:
                    ...
                    macrodict["i_calib_source"] = values[1]
                    macrodict["i_proc_dir"] = values[2]
        ...
        macrodict['transform'] = transform
        macrodict["stitch"] = stitch
        macrodict["integrate"] = integrate
        ...
        if integrate:
            ...
            if not os.path.exists(i_calib_source):
                displayError(self, "Please select an integrate calibration source!")
                return
...
def beginQueue(self):
    ...
    for macro in self.macroQueue:
        ...
        if macro.isWorkflow():
            ...
            startIntegrateThread(self, dataFiles, calib_source, detectorData, i_processed_filedir, QRange, ChiRange, increment)
            while self.processDone == False:
                time.sleep(.2)
                QApplication.processEvents()
            time.sleep(1.5)
            ...
            if macro.shouldIntegrate():
                ...
                QRange = macro.getQRange()
                ChiRange = macro.getChiRange()
                startIntegrateThread(self, dataFiles, calib_source, detectorData, processed_filedir, QRange, ChiRange, increment)      
                while self.processDone == False:
                    time.sleep(.2)
                    QApplication.processEvents()     
                time.sleep(1.5)                
...
# list str tuple str tuple tuple float-> None
# Takes a list of datafiles (either a list with a string folder name inside or a list of pathnames), a calibration source, a tuple of detector data, a processed file directory, a Q Range, and a Chi Range, and starts the integrate thread. The float is the increment to push the queue radial bar
def startIntegrateThread(self, dataFiles, calib_source, detectorData, processed_filedir, QRange, ChiRange, increment):
    if self.changeTabCheck.isChecked():
    ...
    self.addToConsole('******************************************************************************')            
    QApplication.processEvents()
    self.integrateThread = IntegrateThread(self, calib_source, processed_filedir, detectorData, dataFiles, (QRange, ChiRange), increment)
